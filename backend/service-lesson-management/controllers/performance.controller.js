...
const getAllPerformanceRecords = async (req, res) => {
	try {
		const results = await performanceModel.find({}).exec();
		if (results) {
			return res.status(200).json(results);
		} else {
			return res
				.status(400)
				.json({ error: "Performance records retrieval failed" });
		}
	} catch (error) {
		console.log(error);
	...
	try {
		...
		if (results) {
			...
			return res
				.status(400)
				.json({
					error: "Performance summary retrieval for the user failed",
				});
		}
	} catch (error) {
		console.log(error);
	...
	try {
		...
		if (results) {
			...
			return res
				.status(400)
				.json({
					error: "Performance retrieval for the lesson failed!",
				});
		}
	} catch (error) {
		console.log(error);
...
const createPerformanceRecord = async (req, res) => {
	...
	if (!userId || !lessonId || !courseId || !performanceScore) {
		return res.status(400).json({
			error: "user id and course id and performanceScore is required for performance record!",
		});
	}
	try {
		...
		if (createdPerformanceRecord) {
			return res
				.status(201)
				.json({
					message: "Performance record created for the target lesson",
				});
		} else {
			return res
				.status(400)
